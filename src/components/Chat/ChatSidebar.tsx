...
import { ChevronLeft, ChevronRight, Loader2, CircleUser, Search } from "lucide-react";
          ...
          <div className="flex-1 min-w-0">
            ...
            {isService && lastMessage && (
              <div className="flex items-center justify-between text-xs text-muted-foreground pr-8">
                ...
                <span className="whitespace-nowrap">{formatTime(lastMessage.timestamp)}</span>
              </div>
            )}
          </div>
        )}
...
const ChatSidebar: React.FC<ChatSidebarProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<"contacts" | ServiceStatus | "groups">("contacts");
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const url = `/whatsapp/service/get-services?status__in=in_progress,pending${debouncedBusca ? `&search=${debouncedBusca}` : ""
          }`;
  ...
  const ContactsList: React.FC<{ contactList: ContactType[] }> = ({ contactList }) => {
    const filtrados = useMemo(() => {
      if (!busca.trim()) return contactList;
      return contactList.filter((c) =>
        c.name.toLowerCase().includes(busca.toLowerCase()) || c.phone_number.includes(busca)
      );
    }, [contactList, busca]);
  ...
  const ServicesList: React.FC<{ list: ServiceType[] }> = ({ list }) => {
    // aplica o filtro pelo nome do contato
    const filtrados = useMemo(() => {
        ...
        >
          <div className="flex-shrink-0 border-b border-border">
            <TabsList className="w-full justify-start rounded-xl h-auto p-0">
              <TabsTrigger value="contacts" className="text-xs flex items-center rounded-xl gap-2">
                <span>Contatos</span>
              </TabsTrigger>
              <TabsTrigger value="in_progress" className="text-xs flex items-center rounded-xl gap-2">
                <span>Em andamento</span>
                <Badge variant="default" className="h-4 px-1 text-xs">
                  {inProgressServices.length}
                </Badge>
              </TabsTrigger>
              <TabsTrigger value="pending" className="text-xs flex items-center rounded-xl gap-2">
                <span>Pendente</span>
            ...
            <TabsList className="w-full justify-start rounded-xl h-auto p-0 mt-2">
              <TabsTrigger value="groups" className="text-xs flex items-center rounded-xl gap-2">
                <span>Grupos</span>
              </TabsTrigger>
            </TabsList>
          ...
          <ScrollArea className="h-full">
            <div className="space-y-2 p-2">
              {loadingContacts ? (
                ...
                <div className="text-center text-sm text-destructive p-2">{contactsError}</div>
              ) : (
                // Quando collapsed, mostrar apenas contatos individuais (não grupos)
