...
const ChatHeader: React.FC<ChatHeaderProps> = ({
  ...
  const isServiceFinalized = currentService?.status === 'finalized';
  const isServiceDismissed = currentService?.status === 'dismiss';
  const isServiceInProgress = currentService?.status === 'in_progress';
  const isGroup: boolean = contact.phone_number.endsWith('@g.us')
      ...
      <div className="flex-shrink-0 border-b border-border bg-card/80 backdrop-blur-sm p-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            <TooltipProvider>
              <div className="flex items-center space-x-2">
                {(isServiceFinalized || !currentService || isServicePending || isServiceDismissed) && !isGroup && (
                  <Tooltip>
